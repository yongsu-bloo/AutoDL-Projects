...
def fitnet_train(xloader, teacher, network, matching_layers, criterion, scheduler, optimizer, optim_config, extra_info, print_freq, logger):
  loss, acc1, acc5 = procedure(xloader, teacher, network, matching_layers, criterion, scheduler, optimizer, 'train', optim_config, extra_info, print_freq, logger)
  return loss, acc1, acc5

def fitnet_valid(xloader, teacher, network, matching_layers, criterion, optim_config, extra_info, print_freq, logger):
  with torch.no_grad():
    loss, acc1, acc5 = procedure(xloader, teacher, network, matching_layers, criterion, None, None, 'valid', optim_config, extra_info, print_freq, logger)
  return loss, acc1, acc5
...
def procedure(xloader, teacher, network, matching_layers, criterion, scheduler, optimizer, mode, config, extra_info, print_freq, logger):
  ...
  for i, (inputs, targets) in enumerate(xloader):
    ...
    _, logits, student_features = network(inputs, out_all=True)
    ...
    with torch.no_grad():
      _, teacher_logits, teacher_features = teacher(inputs, out_all=True)

    # loss             = loss_KD_fn(criterion, logits, teacher_logits, student_f, teacher_f, targets, config.KD_alpha, config.KD_temperature)
    loss = loss_fitnet_fn(teacher_features, student_features, matching_layers)
