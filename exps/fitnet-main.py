...
from nas_infer_model import obtain_nas_model_by_genotype
...
def main(args):
  ...
  supernet_config = dict2config({'name': 'SETN', 'C': args.channel, 'N': args.num_cells,
  ...
  # load checkpoint
  # Teacher
  # teacher_checkpoint = torch.load( args.teacher_checkpoint )
  # xargs      = teacher_checkpoint['args']
  # teacher_model_config = dict2config(teacher_checkpoint['model-config'], logger)
  # teacher_base   = obtain_model(teacher_model_config)
  # teacher_optim_config = dict2config(teacher_checkpoint['optim-config'], logger)
  # _, _, teacher_criterion = get_optim_scheduler(teacher_base.parameters(), teacher_optim_config)
  # teacher_base.load_state_dict( teacher_checkpoint['base-model'] )
  teacher_base = load_net_from_checkpoint(args.teacher_checkpoint)
  teacher      = torch.nn.DataParallel(teacher_base).cuda()

  #Student
  search_model = get_cell_based_tiny_net(supernet_config) # trained
  student_checkpoint  = torch.load(args.student_checkpoint)
  search_model.load_state_dict( student_checkpoint['search_model'] )
  student_model = get_cell_based_tiny_net(supernet_config)
  # load supernet
  ...
  w_optimizer, w_scheduler, criterion = get_optim_scheduler(student_model.get_weights(), optim_config)
  criterion = criterion.cuda()
  # teacher_criterion = teacher_criterion.cuda()
  h_optimizer, h_scheduler, _ = get_optim_scheduler(list(student_model.get_weights()) + list(matching_layers.parameters()), matching_optim_config)
  # logger.log('Teacher ====>>>>:\n{:}'.format(teacher_base))
  ...
  if last_info.exists() and not args.overwrite: # automatically resume from previous checkpoint
    ...
    if args.sample_method == 'best':
        ...
        genotype = search_model.dync_genotype(use_random=True) # sample uniformly
    else:
        raise ValueError('invalid sample_method: {:}'.format(args.sample_method))
    # Normal Training result from nas102
  ...
  del search_model, student_checkpoint
  network.module.set_cal_mode('dynamic', genotype)
  logger.log("-"*100 + "\nNormal Training Result: \n Train : {:}\n Valid : {:}\n".format(*nor_train_results) + "-"*100)

  """Main Training and Evaluation Loop"""
  # Hint Training First
  start_time, search_time, epoch_time, total_epoch = time.time(), AverageMeter(), AverageMeter(), matching_optim_config.epochs + matching_optim_config.warmup
  ...
  for epoch in range(start_epoch, total_epoch):
    ...
    # log the results
    logger.log('***{:s}*** HINT TRAIN [{:}] loss = {:.6f}'.format(time_string(), epoch_str, train_loss))
  ...
  start_time, search_time, epoch_time, total_epoch = time.time(), AverageMeter(), AverageMeter(), optim_config.epochs + optim_config.warmup
  if args.exp_name == "tmp": total_epoch = 2
  ...
  for epoch in range(start_epoch, total_epoch):
    ...
    save_path = save_checkpoint({
          ...
          'nor_train_results'   : deepcopy(nor_train_results),
...
if __name__ == '__main__':
  ...
  # Save and Load
  parser.add_argument('--save_dir',           type=str,   default="./output/sample-train",     help='Folder to save checkpoints and log.')
  ...
  parser.add_argument('--sample_method',    type=str,     default='best', choices=['uniform', 'infer', 'best'],    help='The architecture sampling method: uniform, infer, best')
  ...
  # Training Options
  parser.add_argument('--procedure'   ,     type=str,        default='Simple-KD',       help='The procedure basic prefix.')
  ...
  if args.sample_method not in args.save_dir:
